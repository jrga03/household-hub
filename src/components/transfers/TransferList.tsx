...
import { OfflineHint } from "@/components/sync/OfflineStates";
...
export function TransferList({ householdId }: { householdId: string }) {
  ...
  if (!transfers || transfers.length === 0) {
    // Offline with nothing mirrored locally: an honest offline state instead
    // of a false "no transfers" claim (review R11)
    if (!isOnline) {
      return (
        <div className="flex flex-col items-center justify-center p-8 text-center">
          <p className="text-muted-foreground">You&apos;re offline</p>
          <p className="text-sm text-muted-foreground mt-1">
            Transfers saved on this device will appear here. Reconnect to load your full transfer
            history.
          </p>
        </div>
      );
