...
Before testing, verify all dependencies are in place:

```bash
# 1. Check sync processor hook exists
grep -l "useSyncProcessor" src/hooks/*.ts
# Expected output: src/hooks/useSyncProcessor.ts

# 2. Check queue count function exists
grep -l "getQueueCount" src/lib/offline/*.ts
# Expected output: src/lib/offline/syncQueue.ts

# 3. Check auth store exists
grep -l "useAuthStore" src/stores/*.ts
# Expected output: src/stores/authStore.ts
```

**If any command shows no results**, complete those prerequisite chunks first before continuing.
...
npm test -- useSyncStatus.test.ts

# Component tests
npm test -- SyncStatus.test.tsx
npm test -- OfflineBanner.test.tsx
npm test -- SyncButton.test.tsx

# Accessibility audit for sync components
npm run test:a11y -- --selector "[data-testid*='sync']"
```

### Expected Test Results

- ✅ All unit tests pass (15/15)
- ✅ All component snapshot tests pass (3/3)
- ✅ No accessibility violations in aXe audit
- ✅ No console errors during render
- ✅ `console.warn` called for pending syncs

### Common Test Failures

**Test fails**: "useSyncProcessor is not defined"

- **Solution**: Complete Chunk 024 first

**Test fails**: "getQueueCount is not a function"

- **Solution**: Complete Chunk 023 first

**Test fails**: "useAuthStore is not defined"

- **Solution**: Complete Chunk 002 first
...
2. **Add Chunk 025-b** (optional): Advanced sync issues panel for conflict visibility
3. Deploy current MVP and iterate
