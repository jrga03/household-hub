...
- [ ] 3.1 `replace: true` in `updateFilters` navigate (R12)
- [ ] 3.2 Mobile `<main>` bottom inset for FAB (R13)
- [ ] 3.3 TransferForm UTC date → `format(new Date(), "yyyy-MM-dd")` + README + unit test (R21)
- [ ] 3.4 Sonner `mobileOffset`/position fix (R7 partial)
- [ ] 3.5 `min-h-11` MobileNav rows + SettingsNav anchors (L)
- [ ] 3.6 Coarse-pointer CSS: select-item/dropdown-item min-height; checkbox/radio `::after` hit expansion; select-trigger (R15, R23, L)
- [ ] 3.7 `w-full` on all form SelectTriggers (R23)
- [ ] 3.8 `prefers-reduced-motion` block in index.css (R34)
- [ ] 3.9 `active:` variants in buttonVariants + MobileNav rows (R16)
- [ ] 3.10 aria-labels on icon-only buttons (~10 sites) (R35)
- [ ] 3.11 Paired theme-color metas + ThemeColorSync + manifest colors (R31)
- [ ] 3.12 StorageWarning top calc + shared stacking container with OfflineBanner (R29)
- [ ] 3.13 MonthSelector "Current" visible on phones (L)
- [ ] 3.14 Account detail onEdit wiring (R25)
- [ ] 3.15 Draft discard: Undo toast (status flip + counter decrement) + gap-2 + destructive styling + aria-labels (R2)
- [ ] 3.16 SummaryCards container-query grid, remove truncate (R4, decision 1)
- [ ] 3.17 Empty-state CTAs (accounts, transactions no-data) (L)
- [ ] 3.18 CurrencyInput commits on change (R22)
- [ ] 3.19 AccountFormDialog + BudgetForm → CurrencyInput + zod cents (R19)
- [ ] 3.20 DatePicker → direct `<Input type="date">`, fix UTC-midnight parse (R20)
- [ ] 3.21 Search input `type="search"` + `enterKeyHint` (C7)
- [ ] 3.22 Offline create toast says "saved on this device, will sync" (L)
